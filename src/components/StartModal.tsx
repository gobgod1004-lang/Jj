...
export const StartModal: React.FC<StartModalProps> = ({
  ...
  const [rankingTab, setRankingTab] = useState<DifficultyLevel>(difficulty);

  // Filter history by the currently selected ranking tab
  const tabHistory = history.filter((h) => h.difficulty === rankingTab);
  ...
  return (
    ...
    >
      <div className="w-full max-w-xl bg-white/95 text-slate-900 border border-amber-200/80 rounded-3xl p-5 sm:p-7 shadow-2xl flex flex-col gap-5 my-auto">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-600 uppercase tracking-wider">
              난이도 선택 (난이도별 순위 별도 측정)
            </span>
            {highScore.score > 0 && (
              <span className="text-xs text-amber-600 flex items-center gap-1 font-bold font-mono">
                <Trophy className="w-3.5 h-3.5" /> {DIFFICULTY_CONFIGS[difficulty].name} 최고: {highScore.score.toLocaleString()}점
              </span>
            )}
          </div>
            ...
            {difficulties.map((lvl) => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col text-left p-3 rounded-2xl border transition-all relative cursor-pointer ${
                    isSelected
        ...
        {/* Play History & Ranking Toggle Accordion (Difficulty-Separated Tabs) */}
        <div className="rounded-2xl border border-slate-200 bg-slate-50 overflow-hidden text-xs">
          <button
            type="button"
            onClick={() => setShowHistory(!showHistory)}
            className="w-full p-3 flex items-center justify-between font-bold text-slate-700 hover:bg-slate-100 transition cursor-pointer"
          >
            <span className="flex items-center gap-1.5">
              <ListOrdered className="w-4 h-4 text-amber-500" />
              <span>난이도별 역대 순위표 보기</span>
            </span>
            {showHistory ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
          </button>

          {showHistory && (
            <div className="p-3 pt-0 flex flex-col gap-2.5 border-t border-slate-200">
              {/* Tabs for Difficulty Ranking */}
              <div className="flex gap-1.5 p-1 bg-slate-200/70 rounded-xl">
                {difficulties.map((diffLvl) => (
                  <button
                    key={diffLvl}
                    type="button"
                    onClick={() => setRankingTab(diffLvl)}
                    className={`flex-1 py-1.5 px-2 rounded-lg font-bold text-xs transition cursor-pointer ${
                      rankingTab === diffLvl
                        ? 'bg-white text-slate-900 shadow-sm'
                        : 'text-slate-600 hover:text-slate-900'
                    }`}
                  >
                    {DIFFICULTY_CONFIGS[diffLvl].name}
                  </button>
                ))}
              </div>
              ...
              {tabHistory.length === 0 ? (
                <div className="p-4 text-center text-slate-400">
                  아직 {DIFFICULTY_CONFIGS[rankingTab].name} 플레이 기록이 없습니다!
                </div>
              ) : (
        ...
        >
          ...
          <span>게임 시작 ({DIFFICULTY_CONFIGS[difficulty].name})</span>
        </button>
      </div>
    </div>
  );
